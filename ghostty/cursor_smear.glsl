...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Trail is always bright blue, independent of the actual cursor color
    // (the cursor rectangle itself is excluded below and rendered natively)
    const vec3 trailColor = vec3(0.1, 0.5, 1.0);
    vec4 enhancedColor = vec4(trailColor, iCurrentCursorColor.a);
